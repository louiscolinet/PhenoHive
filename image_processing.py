...
def match_luminance(img: np.ndarray, background: np.ndarray, radius: int = 150) -> np.ndarray:
    """
...
def remove_shadows(img: np.ndarray, beta1=0.38, beta2=0.85, tau_s=0.27, tau_h=1) -> np.ndarray:
    """
    ...
    shadow_mask_blurred = cv2.GaussianBlur(shadow_mask_uint8, (5, 71), 0)
    ...
    if nb_white_pixels > 20000:
        ...
        eroded_mask = cv2.erode(shadow_mask_u8, np.ones((60, 60), np.uint8), iterations=1)
        dilated_mask = cv2.dilate(shadow_mask_u8, np.ones((7, 7), np.uint8), iterations=1)
        transition_mask = cv2.subtract(dilated_mask, eroded_mask)
        blurred = cv2.GaussianBlur(img_no_shadow, (5, 91), 0)
        img_no_shadow[transition_mask.astype(bool)] = blurred[transition_mask.astype(bool)]

        img_no_shadow = cv2.bilateralFilter(img_no_shadow, d=15, sigmaColor=35, sigmaSpace=95)
    else:
        img_no_shadow = img.copy()
